...
def template_from_images(images):
    ...
    for img in images.keys():
        ...
        if deg!=0 or brightness!=1:
            locals()[img] = rotate_resize_brightness(images[img]['path_to_img'], img[:-4], deg, img_size, images[img]['size'], brightness)
...
images = {
    'back_img':{
        'path_to_img': "generated_images/back.png",
        'size': 1, # коэффициет (1 - не менять размер)
        'pos':(0, 0), # px
        'rotation': 0, # градусов
        'blur': (20, 20), # размер блюра
    ...
    'obj_shadow_img':{ # тень для объекта - то же изображение, но с блюром
        'path_to_img': "generated_images/obj.png",
        'size': 0.8,
...
final_template = template_from_images(images)
