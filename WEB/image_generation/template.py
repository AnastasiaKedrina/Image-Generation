...
def save_final_template(final_template, img_size, aspect_ratio):
    uniq_filename = str(datetime.datetime.now().date()) + '_' + str(datetime.datetime.now().time()).replace(':', '-')
    cv2.imwrite(f'image_generation/result_templates/all/{uniq_filename}.png', final_template)
    cv2.imwrite(f'image_generation/result_templates/result_{aspect_ratio}.png', final_template)
    print(f'---ШАБЛОН {img_size} СОХРАНЕН')

# --------------------------------------------------------
def create_portrait_template_1(back_img_path, obj_img_path, aspect_ratio, img_size, fonts, colors, text_data):
    
    texts = {
        'title_text':{
            'text':add_line_break(text_data['title_text'], 7), # число символов в одной строке до переноса
        ...
        'subtitle_text':{
            'text':add_line_break(text_data['subtitle_text'], 20),
        ...
        'address_text':{
            'text':add_line_break(text_data['address_text'], 20),
    ...
    images = {
        'aspect_ratio':{
            'path_to_img': f'image_generation/template_elements/portrait.png',
            'size': 1,
            'pos':(0, 0),
            'rotation': 0,
            'blur': (0, 0),
        ...
        'white_back_img':{
            'path_to_img': "image_generation/template_elements/portrait_diagonal.png",
            'size': 1,
            'pos':(0, 0),
            'rotation': 0, 
            'blur': (0, 0),
        ...
        'white_button_img':{
            'path_to_img': "image_generation/template_elements/white_button.png",
            'size': 1,
    ...
    save_final_template(final_template, img_size, 'portrait')

# --------------------------------------------------------
def create_square_template_1(back_img_path, obj_img_path, aspect_ratio, img_size, fonts, colors, text_data):
    texts = {
        'title_text':{
            'text':add_line_break(text_data['title_text'], 7), # число символов в одной строке до переноса
            # 'pos':(get_position('left', 'top', 1, 1)), # отсчет от верхнего левого края, (1,1) начальные коожинаты сетки
    ...
    images = {
        'aspect_ratio':{
            'path_to_img': f'image_generation/template_elements/square.png',
            'size': 1,
            'pos':(0, 0),
            'rotation': 0,
            'blur': (0, 0),
        ...
        'white_back_img':{
            'path_to_img': "image_generation/template_elements/square_circle.png",
            'size': 1,
            'pos':(0, 0),
            'rotation': 0, 
            'blur': (0, 0),
    ...
    final_template = template_from_images(images, img_size)
    final_template = paste_text_images(final_template, texts)
    save_final_template(final_template, img_size, 'square')

# --------------------------------------------
def create_square_template_2(back_img_path, obj_img_path, aspect_ratio, img_size, fonts, colors, text_data):
    pass

# ------------

# --------------------------------------------------------
def create_landscape_template_1(back_img_path, obj_img_path, aspect_ratio, img_size, fonts, colors, text_data):
    texts = {
        'title_text':{
            'text':add_line_break(text_data['title_text'], 7), # число символов в одной строке до переноса
            # 'pos':(get_position('left', 'top', 1, 1)), # отсчет от верхнего левого края, (1,1) начальные коожинаты сетки
    ...
    images = {
        'aspect_ratio':{
            'path_to_img': f'image_generation/template_elements/landscape.png',
            'size': 1,
            'pos':(0, 0),
            'rotation': 0,
            'blur': (0, 0),
        ...
        'white_back_img':{
            'path_to_img': "image_generation/template_elements/landscape_side.png",
            'size': 1,
            'pos':(0, 0),
            'rotation': 0, 
            'blur': (0, 0),
    ...
    final_template = template_from_images(images, img_size)
    final_template = paste_text_images(final_template, texts)
    save_final_template(final_template, img_size, 'landscape')
